    ...
    return StreamBuilder<List<EventTag>>(
      ...
      builder: (context, tagSnapshot) {
        if (tagSnapshot.hasError) {
          return const Center(
            child: Text('Unable to load tags. Please try again later.'),
          );
        }
        ...
        return StreamBuilder<TagRoutingSettings>(
          ...
          builder: (context, settingsSnapshot) {
            if (settingsSnapshot.hasError) {
              return const Center(
                child: Text(
                  'Unable to load tag rules. Please try again later.',
                ),
              );
            }
            ...
            return Column(
              children: [
                SwitchListTile(
                  ...
                  subtitle: const Text(
                    'Apply the rules below to events that have no tag yet.',
                  ),
                              ...
                              trailing: Row(
                                ...
                                children: [
                                  ...
                                  IconButton(
                                    ...
                                    onPressed: () =>
                                        _deleteRule(context, settings, index),
                                  ),
                                ],
                              ),
                            );
...
class _TagRuleDialogState extends State<_TagRuleDialog> {
  ...
  void initState() {
    ...
    _selectedTagId =
        widget.existing?.tag ??
        (widget.tags.isEmpty ? null : widget.tags.first.id);
  }
