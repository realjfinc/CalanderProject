...
class FirestoreTagRoutingRepository implements TagRoutingRepository {
  factory FirestoreTagRoutingRepository({required String uid, FirebaseFirestore? firestore}) {
    return FirestoreTagRoutingRepository._(uid, firestore ?? FirebaseFirestore.instance);
  }
  ...
  DocumentReference<Map<String, dynamic>> get _settingsRef =>
      _firestore.collection('users').doc(_uid).collection('settings').doc('tagRouting');

  @override
  Stream<TagRoutingSettings> watchSettings() {
    return _settingsRef.snapshots().map((snapshot) {
      final data = snapshot.data();
      return data == null ? const TagRoutingSettings() : TagRoutingSettings.fromMap(data);
    });
  }

  @override
  Future<void> updateSettings(TagRoutingSettings settings) {
    return _settingsRef.set(settings.toMap());
  }
}
